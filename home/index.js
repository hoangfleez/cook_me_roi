...
function getUser() {
    $.ajax({
        type: 'GET', url: `http://localhost:3000/auth/home`, headers: {
            "Content-Type": "application/json", 'Authorization': 'Bearer ' + localStorage.getItem('authorization')
        }, success(data) {
            $.ajax({
                type: "GET", url: `http://localhost:3000/friend`, headers: {
                    "Content-Type": "application/json",
                    'Authorization': 'Bearer ' + localStorage.getItem('authorization')
                }, success(friendList) {
                    $.ajax({
                        type: "GET", url: `http://localhost:3000/post`, headers: {
                            "Content-Type": "application/json",
                            'Authorization': 'Bearer ' + localStorage.getItem('authorization')
                        }, success(postList) {
                            $.ajax({
                                type: "GET", url: `http://localhost:3000/like/id`, headers: {
                                    "Content-Type": "application/json",
                                    'Authorization': 'Bearer ' + localStorage.getItem('authorization')
                                }, success: (likes) => {
                                    let listIdPost = []
...
function like(id, length) {
    $.ajax({
        type: 'POST', url: `http://localhost:3000/like?id=${id}`, headers: {
            "Content-Type": "application/json", 'Authorization': 'Bearer ' + localStorage.getItem('authorization')
        }, success: () => {
...
function unLike(id, length) {
    $.ajax({
        type: 'DELETE', url: `http://localhost:3000/like?id=${id}`, headers: {
            "Content-Type": "application/json", 'Authorization': 'Bearer ' + localStorage.getItem('authorization')
        }, success: () => {
 ...
 `;
    ...
    function uploadImage(e) {
        ...
        uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, function (snapshot) {
            ...
            document.getElementById("imgDiv").innerHTML = `<img src="${downloadURL}" alt="" style="width: 450px; height: 180px" >`;
    ...
    function showCommentBox(id) {
        ...
        $.ajax({
            type: 'GET', url: `http://localhost:3000/post/comment?id=${id}`, headers: {
                "Content-Type": "application/json", 'Authorization': 'Bearer ' + localStorage.getItem('authorization')
            }, success(data) {
                $.ajax({
                    type: 'GET', url: `http://localhost:3000/comment/post?id=${id}`, headers: {
                        "Content-Type": "application/json",
                        'Authorization': 'Bearer ' + localStorage.getItem('authorization')
                    }, success(comments) {
                        let listComment = ``
